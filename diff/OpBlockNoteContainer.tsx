...
export default function OpBlockNoteContainer({ inputField,
  ...
  // Countdown while waiting for the collaboration server to connect
  useEffect(() => {
    if (!errorContainer || !isLoading || connectionError) return;

    let seconds = 1;
    const update = () => {
      errorContainer.innerHTML =
        `<span style="color:#888;font-size:13px;">Die Verbindung wird aufgebaut\u2002\u2002${seconds}\u00a0s</span>`;
    };
    update();
    const timer = setInterval(() => { seconds++; update(); }, 1000);

    return () => {
      clearInterval(timer);
      errorContainer.innerHTML = '';
    };
  }, [isLoading, connectionError, errorContainer]);
